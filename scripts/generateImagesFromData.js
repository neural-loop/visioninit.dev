...
const DEFAULT_PRIMARY_COLOR_FALLBACK = '#748091'; // Fallback if not in JSON
...
async function generateImages() {
  console.log('🖼️ Starting OG image generation from collected data...');

  // 1. Verify dependencies
  if (!(await pathExists(TEMPLATE_PATH))) { /* ... error handling ... */ process.exit(1); }
  if (!(await pathExists(INPUT_JSON_PATH))) { /* ... error handling ... */ process.exit(1); }
  ...
  if (!jsonData || !jsonData.pages || jsonData.pages.length === 0) {
    console.warn('⚠️ No pages found in the data file. Nothing to generate.');
    return;
  }

  // --- Get global values from JSON ---
  const logoDataUri = jsonData.logoDataUri || '';
  const siteNameForDisplay = jsonData.siteNameForDisplay || 'visioninit.dev'; // Use collected URL/name
  const primaryColor = jsonData.colors?.['primary-color'] || DEFAULT_PRIMARY_COLOR_FALLBACK; // Get primary from palette
  // You could access other colors like: const textColor = jsonData.colors?.['text-color'];
  ...
  for (const pageData of jsonData.pages) {
    ...
    if (!title || !description || !outputPath || !tempHtmlPath) { /* ... skip logic ... */ continue; }
    ...
    try {
      // Populate template using values from JSON
      const htmlContent = templateContent
        .replace('LOGO_SRC', logoDataUri) // Use logo data from JSON root
        .replace('PAGE_TITLE', title)
        .replace('PAGE_DESCRIPTION', description)
        .replace('SITE_NAME', siteNameForDisplay) // << Use site URL/name from JSON root
        .replace('PRIMARY_COLOR_PLACEHOLDER', primaryColor); // << Use primary color from JSON root

      debugLog(`  Writing temp HTML...`);
      await writeFile(tempHtmlPath, htmlContent);

      const command = `wkhtmltoimage --enable-local-file-access --quality 90 --width 1200 --height 630 "${tempHtmlPath}" "${outputPath}"`;
      debugLog(`  Executing: ${command}`);
      execSync(command, { stdio: 'pipe' });

      console.log(`✅ Generated OG image for: ${pageIdentifier}`);
      successCount++;
      ...
      console.error(`❌ Failed generating image for ${pageIdentifier}:`, err.stderr?.toString() || err.message);
